...
class App extends Component {
  ...
  eliminar = (clave) => {
    let temporal = this.state.horario.filter((c, mat) => c !== clave)
  ...
  buscar = (materia) => {
    let existe = this.state.horario.some(
      mat => mat.clave === materia.clave || mat.hora === materia.hora || mat.materia === materia.materia
    );
    return existe;
  }
  ...
  nulo = (materia) => {
    let existe = (
      materia.clave === "N/A" || materia.hora === "N/A" || materia.materia === "No hay materias disponibles"
    );
    return existe;
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        {/* Borra línea 71 de "materias" en "Courses" para activar defaultProps */}
          <Courses 
